    ...
    <div v-for="product in products" :key="product.id" @click="open(product)" class="container shadow-sm">
      <transition>
        <div class="preview" v-show="product.show">
          <img :src="product.image" style="position: relative; z-index: 1; margin: 2%; user-select: none;" :style="{ backgroundColor: product.color?.color ? product.color?.color : '#fff' }" width="400px">
          <img
            id="symbol"
            ref="symbol"
            :src="product.symbolImage"
            width="100px"
            :style="{ top: product.symbolPos.y + 'px', left: product.symbolPos.x + 'px' }"
          >
        </div>
      </transition>
...
export default {
  ...
  async fetch () {
    ...
    this.$store.state.cart.forEach((product) => {
      ...
      this.symbols.map((acc) => {
        if (acc.id === product.symbol) {
          output.symbolImage = acc.image
        }
        return acc
      })
